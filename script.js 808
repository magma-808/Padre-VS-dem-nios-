...
class Enemy {
    ...
    draw(ctx) {
        ...
        ctx.fillStyle = '#000';
        ctx.fillRect(this.x + 5, this.y - 10, 5, 10); ctx.fillRect(this.x + this.w - 10, this.y - 10, 5, 10);
        ctx.fillStyle = '#fff';
        ctx.fillRect(this.x + 10, this.y + 10, 5, 5); ctx.fillRect(this.x + this.w - 15, this.y + 10, 5, 5);
        if (this.hp > 1) {
...
const game = {
    ...
    init() {
        ...
        if (indicator) indicator.addEventListener('click', () => this.cycleCross());
    ...
    startPhase(p) {
        ...
        Object.values(CROSS_TYPES).forEach(t => {
            if (this.phase >= t.phase && !this.player.unlockedCrosses.includes(t.id)) {
                this.player.unlockedCrosses.push(t.id);
                this.player.selectedCross = t;
            }
        });
        ...
        if (transitionScreen) {
            ...
            setTimeout(() => {
                ...
                this.state = 'PLAYING'; this.updateHUD();
            }, 2000);
    ...
    areaDamage(x, y, radius, damage) {
        this.enemies.forEach(e => {
            if (Math.hypot(e.x + e.w/2 - x, e.y + e.h/2 - y) < radius) {
                e.hp -= damage; if (e.hp <= 0) e.active = false;
            }
        });
    },
    ...
    restart() { document.querySelectorAll('.screen').forEach(s => s.classList.add('hidden')); this.start(); },
    showMenu() { this.state = 'MENU'; document.querySelectorAll('.screen').forEach(s => s.classList.add('hidden')); document.getElementById('menu-screen').classList.remove('hidden'); document.getElementById('hud').classList.add('hidden'); },
    showControls() { document.getElementById('menu-screen').classList.add('hidden'); document.getElementById('controls-screen').classList.remove('hidden'); }
};
